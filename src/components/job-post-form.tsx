...
export default function JobPostForm({ locale, userId }: JobPostFormProps) {
  ...
  const [jobType, setJobType] = useState("full-time-temp");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/${locale}/dashboard/jobs/${data[0].id}`);
    } catch (error: any) {
      toast({
